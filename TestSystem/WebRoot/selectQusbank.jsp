...
<body class=""> 
 	<%
    	...
    	List<Qusbank> qusbankList = (List<Qusbank>)request.getAttribute("qusbankList");
  ...
  <!--<![endif]-->
    ...
    <div class="sidebar-nav">
        ...
        <ul id="legal-menu" class="nav nav-list collapse">
            ...
            <li class="active"><a href="SelectQusbankSer?pageNo=1">试题操作</a></li>
        </ul>
    </div>
     <div class="content">
...
<div class="btn-toolbar">
    <button id="addQus_btn" class="btn btn-primary"><i class="icon-plus"></i> 添加试题</button>
  <div class="btn-group">
  </div>
</div>
<div class="well">
    <table class="table">
      ...
      <%
      	int i=1;
      	if(qusbankList!=null && qusbankList.size()>0){
      	for(Qusbank qus : qusbankList){
     	%>
        <tr>
         <td> <%=i++ %> </td>
          <td>
          	<%if(qus.getQusissue().length()>20){ %>
          		<%=qus.getQusissue().substring(0,20)+"..." %>
          	<%}else{ %>
          	<%=qus.getQusissue() %>
          	<%} %>
          </td>
          <td><%=qus.getQusanswer() %></td>
          <td><%=qus.getQustype() %></td>
          <td>
              <a href="UpdateQusbankSer?qusid=<%=qus.getQusid() %>"><i class="icon-pencil"></i></a>
              <input type="hidden" id="qusid<%=i-1 %>" value="<%=qus.getQusid()%>">
...
<div class="pagination">
    ...
    %>
    	 <li><a>上一页</a></li>
    	 <li><a href="SelectQusbankSer?pageNo=<%=++pagenumber %>">下一页</a></li>
    <%
    	}else if(pagenumber >= totalPage){
    %>
     	<li><a href="SelectQusbankSer?pageNo=<%=--pagenumber %>">上一页</a></li>
    	<li><a>下一页</a></li>
    <%
    	}else{ 
    	Integer p = pagenumber;
    %>
        <li><a href="SelectQusbankSer?pageNo=<%=--p %>">上一页</a></li>
        <li><a href="SelectQusbankSer?pageNo=<%=++pagenumber %>">下一页</a></li>
    <%
    	} 
    %>
	...
	<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	    ...
	    <div class="modal-footer">
	        ...
	        <button class="btn btn-danger" data-dismiss="modal" id="delQusconfirm">确定</button>
	    </div>
	</div>
       </div>
   </div>
