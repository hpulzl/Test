...
<%@page import="qb.entity.QusTestPaper"%>
<%
...
<body class=""> 
 	<%
    	...
    	List<QusTestPaper> list =(List<QusTestPaper>)request.getAttribute("list");
  ...
  <!--<![endif]-->
    ...
    <div class="sidebar-nav">
        ...
        <ul id="error-menu" class="nav nav-list collapse">
            ...
            <li class="active"><a href="selectQusandTestSer?pageNo=1">一键生成试卷</a></li>
        </ul>
...
<div class="well">
    <table class="table" id="createTable">
      ...
      if(list!=null&&list.size()>0){
      	int i=1;
        for(QusTestPaper qustest : list){
        %>	
        <tr>
         <td><%=i++%></td>
          <td><%=qustest.getTestname()%></td>
          <td><%=qustest.getTestdifficult()%></td>
          <td>30</td>
          <td>
          	<%if(qustest.getIsCreated()){ %>
          	  <button class="btn btn-small btn-info" disabled="disabled">已生成试卷</button>
        	<%}else{ %>
        	<button class="btn btn-small btn-success" id="createTest<%=i-1 %>" onclick="create('<%=qustest.getTestid() %>')">未生成试卷</button>
        	<%} %>
          </td>
          <td>
           <a href="#myModal" role="button" data-toggle="modal">
              <input type="hidden" id="testqusid<%=i-1 %>" value='<%=qustest.getTestid() %>'>
...
<div class="pagination">
    ...
    %>
    	 <li><a>上一页</a></li>
    	 <li><a href="selectQusandTestSer?pageNo=<%=++pagenumber %>">下一页</a></li>
    <%
    	}else if(pagenumber >= totalPage){
    %>
     	<li><a href="selectQusandTestSer?pageNo=<%=--pagenumber %>">上一页</a></li>
    	<li><a>下一页</a></li>
    <%
    	}else{ 
    	Integer p = pagenumber;
    %>
        <li><a href="selectQusandTestSer?pageNo=<%=--p %>">上一页</a></li>
        <li><a href="selectQusandTestSer?pageNo=<%=++pagenumber %>">下一页</a></li>
    <%
    	} 
    %>
	...
	<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	    ...
	    <div class="modal-footer">
	        ...
	        <button class="btn btn-danger" data-dismiss="modal" id="delCreateTestpaper">确定</button>
	    </div>
	</div>
       </div>
   </div>
